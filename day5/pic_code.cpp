...
int init() { //developer Sruti
    ...
    for (int j = 0; j < ncols; ++j) {
         //if (j%2==0) {
         ParArray[0][j]=0.000001;  // TODO position 
    }
    

    for (int j = 0; j < ncols; j++) {   //ions
         if (j%2==0) {
             ParArray[1][j]=0.0; //TODO charge [+1, (rho*dx)] //kdm changes introduced division by nppc
             ParArray[2][j]=1.0; //TODO mass [(1*dx)]
         }
         else {   //elctrons
             ParArray[1][j]=0.0; //TODO charge [-1, (rho*dx)]
             ParArray[2][j]=1.0; //TODO  mass [(dx/mass ratio)----electron]
         }
    }
